...
/* ===== Admin Login (only via /admin.html) ===== */
...
async function deleteProduct(id) {
  try {
    ...
    if (res.ok) { alert("Product deleted!"); loadAdminProducts(); if (document.getElementById("product-list")) loadProducts(); }
    else { const data = await res.json(); alert(data.message || "Failed to delete product"); }
...
async function deleteOrder(id) {
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/orders/${id}`, { method: "DELETE", headers: { "Content-Type": "application/json" } });
    if (res.ok) { alert("Order deleted successfully!"); loadAdminOrders(); }
    else { const data = await res.json(); alert(data.message || "Error deleting order"); }
